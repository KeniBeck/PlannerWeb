...
export default function Users() {
  const { users, loading, addUser, updateUser, deleteUser, refreshData } =
    useUsers();
  ...
  const [userToChangePassword, setUserToChangePassword] = useState<
    User | undefined
  >(undefined);
  const [areaToActivate, setAreaToActivate] = useState<User | null>(null);
  const [areaToDelete, setAreaToDelete] = useState<User | null>(null);
  ...
  const filteredUsers = useMemo(
    () =>
      users.filter((user) => {
        ...
        // Filtro por rol
        const matchesRole =
          roleFilter === "all" || user.occupation === roleFilter;
  ...
  const confirmActivateUser = () => {
    ...
    StatusSuccessAlert("Éxito", "Usuario activado correctamente");
  };
  ...
  const actions: TableAction<User>[] = useMemo(
    () => [
      ...
      {
        label: (user) => (user.status === "ACTIVE" ? "Eliminar" : "Activar"),
        icon: (user) =>
          user.status === "ACTIVE" ? (
