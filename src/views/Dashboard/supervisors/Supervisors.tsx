...
export default function Supervisors() {
  const { users, loading, refreshData, deleteUser, updateUser } = useUsers();
  ...
  const [supervisorToActivate, setSupervisorToActivate] = useState<User | null>(
    null
  );
  const [supervisorToDelete, setSupervisorToDelete] = useState<User | null>(
    null
  );
  ...
  const handleActivateClick = (user: User) => {
    setSupervisorToActivate(user);
  };

  const handleDeleteClick = (user: User) => {
    setSupervisorToDelete(user);
  };

  const ConfirmActivateSupervisor = () => {
    if (!supervisorToActivate) return;

    updateUser({
      ...supervisorToActivate,
      status: "ACTIVE",
    });
  };
  ...
  return (
    <>
      ...
      <ActivateItemAlert
        open={!!supervisorToActivate}
        onOpenChange={() => setSupervisorToActivate(null)}
        onConfirm={ConfirmActivateSupervisor}
        itemName="supervisor"
        isLoading={loading}
      />
      ...
      <DeactivateItemAlert
        ...
        onOpenChange={() => setSupervisorToDelete(null)}
